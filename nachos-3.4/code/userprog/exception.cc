...
void ExceptionHandler(ExceptionType which)
{
	...
	case SyscallException:
		...
		case SC_PrintString:
		{
			...
			int virtAddr;
			char* buffer;
			virtAddr = machine->ReadRegister(4); // Lay dia chi cua pace sang System Space voi bo dem buffer dai 255 ki tu
			int length = 0;//tham so buffer tu thanh ghi so 4
			buffer = User2System(virtAddr, 255); // Copy chuoi tu vung nho User S
